...
[name]: use-web-page-to-plot-signals-with-python
[tag]: Python, signal plotting | 訊號繪製, Tornado, jQuery, Flot, web dev | 網頁開發, MPU6050
[photo]: http://i.minus.com/jVm1xxkcGcPog.png
-->

![MPU6050 Web Plot][feature photo]

之前使用過 Python 的 matplotlib 套件來繪製，可惜僅能事先錄製好訊號，再將存放訊號的檔案透過 matplotlib 來繪製訊號圖。這樣的方式除了愚蠢外，也不便於硬體開發，因此打算寫一個 GUI 程式並可以實時 (real-time) 的繪製出訊號。
...
完成後打開網頁就會發現有訊號產生了，只是會一直亂跳。因為我們去除了範例的亂數產生部分，而沒有給定初始訊號值。其實就只要一個包含 100 個 0 的陣列給 Flot 即可。其餘關於 Flot 的樣式可以參考 Flot 的官方文件－[Flot Reference][7]。
